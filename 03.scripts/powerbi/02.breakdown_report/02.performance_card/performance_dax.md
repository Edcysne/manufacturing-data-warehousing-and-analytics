...
// ---------------------------------------------------------
// 🟩 [STEP 1] - DATA CONNECTED TO THE MODEL (OEE PERFORMANCE)
// ---------------------------------------------------------

-- Performance weighted by run_time (consistent with the OEE calculation).
VAR _value_current =
    DIVIDE (
        SUMX ( fact_status_table_dev, fact_status_table_dev[performance] * fact_status_table_dev[run_time] ),
        SUM ( fact_status_table_dev[run_time] )
    )

VAR value_num                      = _value_current        -- Actual: current performance (e.g. 1.05 = 105%)
VAR target_num                     = 0.70              -- Fixed default target of 70%
VAR label_txt                      = "Performance"             -- Main card title
