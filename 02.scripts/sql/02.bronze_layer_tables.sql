...
DROP TABLE IF EXISTS bronze_layer.breakdown_data;

-- Creation of the first table
CREATE TABLE bronze_layer.breakdown_data (
    ID                       VARCHAR(250) PRIMARY KEY,
    date                     VARCHAR(50),    
    line                     VARCHAR(100), 
    event_time               VARCHAR(10),
    unplanned_downtime       VARCHAR(10),
    planned_downtime         VARCHAR(10), 
    work_station             VARCHAR(50),
    machine                  VARCHAR(50),
    failure_type             VARCHAR(50),
    sub_failure_type         VARCHAR(50),
    failure_description      VARCHAR(MAX)    
);

-- Security Layer for Database Existance Checking
DROP TABLE IF EXISTS bronze_layer.status_data;

-- Creation of the second table
CREATE TABLE bronze_layer.status_data (
    ...
    shift                VARCHAR(10),
    leader               VARCHAR(30),
    num_operators        VARCHAR(10),
    total_produced       VARCHAR(10),
    nok_parts            VARCHAR(10),
    reworked_parts       VARCHAR(10),
...
DROP TABLE IF EXISTS bronze_layer.file_log;
CREATE TABLE bronze_layer.file_log (
