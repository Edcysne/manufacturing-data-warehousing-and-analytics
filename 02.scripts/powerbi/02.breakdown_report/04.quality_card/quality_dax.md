...
// ---------------------------------------------------------
// 🟩 [STEP 1] - DATA CONNECTED TO THE MODEL (OEE QUALITY)
// ---------------------------------------------------------

-- Quality weighted by run_time (consistent with the OEE calculation).
VAR _value_current =
    DIVIDE (
        SUMX ( fact_status_table_dev, fact_status_table_dev[quality] * fact_status_table_dev[run_time] ),
        SUM ( fact_status_table_dev[run_time] )
    )

VAR value_num                      = _value_current        -- Actual: current quality (e.g. 0.96 = 96%)
VAR target_num                     = 0.95              -- Fixed default target of 95%
VAR label_txt                      = "Quality"             -- Main card title
